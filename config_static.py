                ...
                while other_index < len(window_tweet)-1:
                    ...
                    for anchor_tweet, other_tweet in zipper(window_tweet[anchor_index:], window_tweet[other_index:]):
                        ...
                        if cosim[0] == 0.0 or cosim[0] == 1.0:
                            ...
                            frame['Window_'+str(window)]['Top Scores'].append(cosim[0]) # update top scores wrt to the anchor
                        else:
                            frame['Window_'+str(window)]['All Scores'].append(cosim[0]) # updtae the frame data structure with the anchor tweet and
                            retained_pair +=1
                    ...
                    q = np.array(frame['Window_'+str(window)]['All Scores']) # convert list of top scores to np array for computational ease
...
if __name__=='__main__':
    ...
    trigger = Static('test.csv',101) # recent name of the class to reflect the the windowing type i.e. Static
    stream = trigger.tweets_stream() # stream of tweets to pull out m batches
    batch = trigger.tweets_batch(stream, window_size=90)#, 313) # batch of tweets from stream of tweets
